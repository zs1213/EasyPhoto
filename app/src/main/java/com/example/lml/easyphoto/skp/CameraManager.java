...
public final class CameraManager {
    ...
    Camera.Size saveOptimalPicSize;
    ...
    private void initCameraParameters(int cameraId, int width, int height) {
        ...
        if (!isEmpty(pictureSizes) && !isEmpty(previewSizes)) {
            Camera.Size optimalPicSize = getMaxOptimalSize(pictureSizes)/*getOptimalSize(pictureSizes, width, height)*/;
            Camera.Size optimalPreSize = getOptimalSize(previewSizes, width, height);
            saveOptimalPicSize = optimalPicSize;
        ...
        try {
            ...
            Camera.Parameters parameterss = mCamera.getParameters();
            if (cameraId == Camera.CameraInfo.CAMERA_FACING_BACK) {
                List<String> focusModes = parameterss.getSupportedFocusModes();
                if (focusModes != null) {
                    if (cameraType == 0) {
                        if (focusModes.contains(Camera.Parameters.FOCUS_MODE_CONTINUOUS_PICTURE)) {
                            parameterss.setFocusMode(Camera.Parameters.FOCUS_MODE_CONTINUOUS_PICTURE);
                        }
                    } else {
                        if (focusModes.contains(Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO)) {
                            parameterss.setFocusMode(Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO);
                        }
                    }
                }
            }
            parameterss.setRotation(90);//设置旋转代码,
            List<Camera.Size> pictureSizess = parameterss.getSupportedPictureSizes();
            List<Camera.Size> previewSizess = parameterss.getSupportedPreviewSizes();
            if (!isEmpty(pictureSizess) && !isEmpty(previewSizess)) {
                Camera.Size optimalPicSize = getMaxOptimalSize(pictureSizess)/*getOptimalSize(pictureSizes, width, height)*/;
                Camera.Size optimalPreSize = getOptimalSize(previewSizess, width, height);
                saveOptimalPicSize = optimalPicSize;
                parameterss.setPictureSize(optimalPicSize.width, optimalPicSize.height);
                parameterss.setPreviewSize(optimalPreSize.width, optimalPreSize.height);
                mProfile.videoFrameWidth = optimalPreSize.width;
                mProfile.videoFrameHeight = optimalPreSize.height;
                mProfile.videoBitRate = 5000000;//此参数主要决定视频拍出大小
            }
            mCamera.setParameters(parameterss);
        }
    }
     ...
     * 释放摄像头
     */
    public Camera.Size getSaveOptimalPicSize() {
        return saveOptimalPicSize;
    }
     ...
     * @return
     */
    private Camera.Size getMaxOptimalSize(List<Camera.Size> sizes) {
        Camera.Size optimalSize = null;
        int value = 0;
        for (Camera.Size size : sizes) {
            int number = size.width * size.height;
            if (number >= value) {
                value = number;
                optimalSize = size;
            }
...
//        optimalSize.height = 1080;
     ...
     * 更换前后置摄像
     */
    public void changeCameraFacing(SurfaceTexture surfaceTexture, int width, int height,String flag) {
        if (isSupportFrontCamera) {
            Camera.CameraInfo cameraInfo = new Camera.CameraInfo();
            int cameraCount = Camera.getNumberOfCameras();//得到摄像头的个数
            for (int i = 0; i < cameraCount; i++) {
                Camera.getCameraInfo(i, cameraInfo);//得到每一个摄像头的信息
                if (cameraFacing == Camera.CameraInfo.CAMERA_FACING_BACK) { //现在是后置，变更为前置
                    if (cameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_BACK) {//代表摄像头的方位为前置
                        closeCamera();
                        cameraFacing = Camera.CameraInfo.CAMERA_FACING_FRONT;
                        CameraUtils.setCameraFacing(context, cameraFacing);
                        openCamera(surfaceTexture, width, height);
...
//                    }
    ...
    public void handleFocusMetering(float x, float y) {
        ...
        try {
            ...
            Toast.makeText(context, "对焦失败", Toast.LENGTH_SHORT).show();
        }
