...
//    private  int MAX_HEIGHT = 1280;
     ...
     * 上传最大图片限制
     */
    private static final int MAX_UPLOAD_PHOTO_SIZE = 3000 * 1024;
     ...
     * 缓存文件根目录名
     */
    private static final String FILE_DIR = "yipai";
    /**
     * 上传的照片文件路径
     */
    private static final String UPLOAD_FILE = "linshi";
     ...
     * @return
     */
    public static boolean savePhoto(Camera.Size optimalPicSize,String photoPath, byte[] data, boolean isFrontFacing, int orientation) {
        if (photoPath != null && data != null) {
            FileOutputStream fos = null;
            try {
                String deviceBrand = SystemUtil.getDeviceBrand();
                if (deviceBrand.equals("samsung")||deviceBrand.equals("Xiaomi")) {
                    orientation += 90;
                }
                Bitmap preBitmap = compressBitmap(data, optimalPicSize.width, optimalPicSize.height);
                if (isFrontFacing) {
                    ...
                    Matrix matrix = new Matrix();
                    matrix.postRotate(orientation);
                    Bitmap newBitmaps = Bitmap.createBitmap(preBitmap, 0, 0, preBitmap.getWidth(), preBitmap.getHeight(), matrix, true);
                    preBitmap = newBitmaps;
                }
//                byte[] newDatas = compressBitmapToBytes(preBitmap, data.length);
                fos = new FileOutputStream(photoPath);
                if(preBitmap.compress(Bitmap.CompressFormat.JPEG, 30, fos))
                {
                    fos.flush();
                    fos.close();
                }
